...
interface IVanActivationProps extends RouteComponentProps<any> {
  alert: any;
  alertClear: any;
  alertError: any;
  alertInfo: any;
...
interface IVanActivationStates {
  fileName: string;
...
class VanActivation extends React.Component<
  IVanActivationProps,
  IVanActivationStates
> {
  constructor(props: IVanActivationProps) {
    super(props);
    this.state = {
      fileName: "",
      isProcessing: false,
  ...
  public readFileOK = (a: any) => {
    if (a.isSuccess) {
      this.props.alertSuccess(
        `File Upload Finish. ${a.totalActivated} Virtual Account are activated`
      );
    } else {
  ...
  public handleFileRead = () => {
    ...
    AsyncPost(
      `${Config.apiUrl}/UobFileUploads/VanActivations`,
      request,
      this.readFileOK,
      this.readFileNotOK,
      this.readFileFailed
